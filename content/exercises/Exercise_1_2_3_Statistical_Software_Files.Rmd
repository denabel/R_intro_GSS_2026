...
allbus_2021 <- read_spss("./data/allbus_2021/ZA5280_v2-0-1.sav")
```

Your collaborators ask you to share the data. Unfortunately, they do not use `SPSS` or `Stata` and, hence, asks you to export your data as a *rds* file.

```{block, box.title = "2", box.body = list(fill = "white"), box.icon = "fa-star"}
Export your data as a *rds* file (its file extension is `.rds`).
```

```{r exportstata, solution = TRUE, eval = FALSE}
saveRDS(allbus_2021, "Allbus_for_R.rds")

# To load
# Allbus_for_R <- readRDS("./data/Allbus_for_R.rds")
```
